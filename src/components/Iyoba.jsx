...
const Iyoba = () => {
  ...
  const [iyobaLegendVisible, setIyobaLegendVisible] = useState(false);
  const [iyobaLegendType, setIyobaLegendType] = useState("idoneidad");
  ...
  const iyobaColors = ["#000000", "#FFA500", "#FFFF00", "#90EE90"];
  ...
  useEffect(() => {
    ...
    const activeIyobaLayers = Object.keys(activeLayers).filter(
      (key) => key.startsWith("rasterIyoba") && activeLayers[key]
    );
    ...
    // Mostrar leyenda de Iyoba si hay alguna capa de idoneidad activa
    if (activeIyobaLayers.length > 0) {
      setIyobaLegendVisible(true);
      setIyobaLegendType("idoneidad");
    } else if (activeImpactoLayers.length > 0) {
      setIyobaLegendVisible(true);
      setIyobaLegendType("impacto");
    } else {
      setIyobaLegendVisible(false);
    }
  }, [activeLayers]);
    ...
    <div style={{ height: "100vh", width: "100%" }}>
      ...
      >
        ...
        {activeLayers.rasterIyoba && (
          <RasterOverlay
            fileName="A_Iyoba_4326.tif"
            colorMap={iyobaColors}
            baseUrl="/"
            continuous={true}
            setError={setError}
            setLoading={setLoading}
            onPixelValue={setPixelValue}
            overlayOpacity={opacity.rasterIyoba}
          />
        )}
        {activeLayers.rasterIyobaCC && (
          <RasterOverlay
            fileName="A_Iyoba_CC_4326.tif"
            colorMap={iyobaColors}
            baseUrl="/"
            ...
            setLoading={setLoading}
            onPixelValue={setPixelValue}
            overlayOpacity={opacity.rasterIyobaCC}
          />
        )}
