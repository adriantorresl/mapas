...
export default function CoreTemplate({ children }) {
  ...
  const handleHeaderNavigation = (pageId) => {
    // El header ya maneja la navegación a través del contexto
  ...
  const displayTitle =
    data?.id === "0"
      ? "Localización"
      : data?.data?.[0]?.content || "Tierra de Agaves";
  const displayBlocks =
    currentBlocks && currentBlocks.length > 0 ? currentBlocks : [];
  ...
  return (
    <div className="core-template">
      <div className="core-template-header">
        ...
        <div className="core-template-map">{children}</div>
      </div>
